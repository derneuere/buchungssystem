// Kalender-Ansicht der Buchungen (Personal) — Monats-/Wochenraster innerhalb
// der Route /admin/buchungen (?ansicht=monat|woche&datum=…).
...
export function BuchungenKalender() {
  ...
  function setAnsicht(neu: 'monat' | 'woche') {
    navigate({ search: (prev) => ({ ...prev, ansicht: neu }) })
  }

  function schritt(delta: -1 | 1) {
    const next = ansicht === 'monat' ? addMonths(anker, delta) : addWeeks(anker, delta)
    navigate({ search: (prev) => ({ ...prev, datum: format(next, 'yyyy-MM-dd') }) })
  }

  function heute() {
    navigate({ search: (prev) => ({ ...prev, datum: undefined }) })
  }
